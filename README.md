...
* **Tasks:**
    ...
    * Runs a lightweight, hardware-optimized Deep Learning inference pipeline to predict **10 different types of hand gestures**. *(Note: Currently executing on the Raspberry Pi 5 CIU, with future plans to port and run inference directly on the STM32H743VIT6 microcontroller).*
    * Implements a Subject-Dependent (SD) classification pipeline consisting of a Convolutional Neural Network (CNN) followed by a Bidirectional Long Short-Term Memory (BiLSTM) network with an Attention mechanism, achieving the following performance metrics across distinct evaluation paradigms:
        * **Subject-Dependent (Intra-Subject):** Attains an inference accuracy of over **91%** when trained and evaluated on isolated, individual user data profiles. (Ref: [single.ipynb](file:///d:/EMG/EMG_Large/sEMG-dataset/Subject_Dependant_Intra_Subject/single.ipynb))
        * **Subject-Dependent (Inter-Subject):** Attains an inference accuracy of over **54%** when evaluated on mixed-population datasets across the entire cohort. (Ref: [global_normalize.ipynb](file:///d:/EMG/EMG_Large/sEMG-dataset/Subject_Dependant-%20Inter_Subject/global_normalize.ipynb))
...
* **Tasks:**
    ...
    * Monitors continuous feedback loops (current sensing and thermal thresholds) to ensure mechanical and user safety.
