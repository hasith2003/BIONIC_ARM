...
def load_per_subject_normalize_split(labeled_dir: Path, window_size=500, step_size=100):
    ...
    def process_files(file_list):
        ...
        for file_path in file_list:
            print(f"   -> Slicing & Per-Subject Normalizing: {file_path.name} ...")
            df = pd.read_csv(file_path, header=None)
            data = df.values.copy()
            
            # Normalization per subject specifically
            for ch in range(4):
